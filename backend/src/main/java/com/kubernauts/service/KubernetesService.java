...
public class KubernetesService {
    ...
    public String rolloutStatus(Long sessionId, String deploymentName) {
        String ns = namespaceFor(sessionId);
        Deployment dep = k8s.apps().deployments().inNamespace(ns).withName(deploymentName).get();
        if (dep == null) return "deployment \"" + deploymentName + "\" not found.";
        ...
        if (ready >= desired && desired > 0)
            return "deployment \"" + deploymentName + "\" successfully rolled out\n" + ready + "/" + desired + " replicas ready.";
        return "Waiting for deployment \"" + deploymentName + "\" rollout to finish: " + ready + "/" + desired + " replicas ready.";
    }

    public String execPod(Long sessionId, String podName, String command) {
        String ns = namespaceFor(sessionId);
        try {
            return k8s.pods().inNamespace(ns).withName(podName)
    ...
    public void labelPod(Long sessionId, String podName, String key, String value) {
        ...
        k8s.pods().inNamespace(ns).withName(podName)
            .edit(p -> new PodBuilder(p)
                .editMetadata().addToLabels(key, value).endMetadata().build());
    }

    public List<Map<String, Object>> getEvents(Long sessionId, String podName) {
        String ns = namespaceFor(sessionId);
        return k8s.v1().events().inNamespace(ns)
